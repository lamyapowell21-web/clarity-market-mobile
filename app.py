...
def render_trade_card(item: dict):
    ...
    label = "EVIDENCE LINE CLEARED" if ready else "WATCH — NOT READY"
...
with settings_tab:
    ...
    if CALIBRATION.get("ready_for_alerts", False):
        ...
        if test.get("positive_rate_lift") is not None:
            st.caption(
                f"Compared with other positive-momentum snapshots in the same untouched period: "
        ...
        st.warning(f"Evidence engine status: {cal_status}. Live evidence-backed alerts stay off until the stricter v12 untouched-test gates all pass.")
    ...
    st.caption(
        "v12 evaluates separated alert events rather than every qualifying five-minute bar. Opportunity and Quick Move remain explanatory only; they do not decide whether Clarity interrupts you."
    )
