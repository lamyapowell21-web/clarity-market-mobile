...
st.markdown(
    ...
    <style>
      ...
      .decision-card.watch {
        background:linear-gradient(135deg,#FFF6C9,#FFF1F7);
        border-color:#E8CBD8;
      }
...
def load_forward_summary() -> dict:
    url = str(CONFIG.get("forward_test_summary_url", "") or "").strip()
    if url:
        try:
    ...
    local = BASE / "forward_test_summary.json"
    if local.exists():
        try:
...
def r_text(x, digits: int = 3) -> str:
    try:
        return f"{float(x):+.{digits}f}R"
    except Exception:
        return "—"


def percentile_text(x) -> str:
    try:
        n = int(round(float(x)))
        if 10 <= n % 100 <= 20:
            suffix = "th"
...
def render_fast_yes(item: dict, mood: dict):
    ...
    strength = percentile_text(evidence.get("evidence_percentile"))
    line = percentile_text(evidence.get("threshold_percentile"))
    reasons = q["reasons"][:2]
    ...
    historical_line = ""
    if test_metrics.get("alerts"):
        historical_line = (
            f"<div class='decision-detail'>Untouched historical test: <strong>{test_metrics.get('alerts', 0)} separated alerts</strong> · "
            f"positive after assumed execution drag <strong>{pct_text(test_metrics.get('positive_rate'))}</strong> · "
            f"average <strong>{test_metrics.get('avg_r', 0):+.2f}R</strong> · "
            f"stress <strong>{test_metrics.get('stress_avg_r', 0):+.2f}R</strong></div>"
        )
    ...
    st.markdown(
        ...
        <div class="decision-card yes">
          ...
          <div class="quick-line">
            <span class="quick-chip">Evidence strength {strength}</span>
            <span class="quick-chip">Alert line {line}</span>
...
def render_fast_watch(item: dict, mood: dict):
    ticker = item["ticker"]
    q, levels, dollars = trade_setup_parts(item)
    evidence = item.get("evidence", {})
    strength = percentile_text(evidence.get("evidence_percentile"))
    horizon = int(CALIBRATION.get("horizon_minutes", 15) or 15)
    reasons = q["reasons"][:2]
    reason_text = " · ".join(reasons) if reasons else "The v13 model crossed its research threshold."

    dollar_line = ""
    if dollars:
        dollar_line = (
            f"<div class='decision-detail'>Paper example using about <strong>{money(dollars['amount'])}</strong>: "
            f"target ≈ <strong>+{money(dollars['target_profit'])}</strong> · "
            f"planned loss ≈ <strong>-{money(dollars['planned_loss'])}</strong></div>"
        )
    ...
    st.markdown(
        ...
        <div class="decision-card watch">
          <div class="decision-kicker">Clarity Watch · paper / research mode</div>
          <div class="decision-answer"><span class="ticker">{ticker}</span> crossed the v13 watch line</div>
          <div class="quick-line">
            <span class="quick-chip">Evidence strength {strength}</span>
            <span class="quick-chip">{horizon}-minute research window</span>
            <span class="quick-chip">Latest {money(q['price'])}</span>
            <span class="quick-chip">Don't chase above {money(levels['entry_ceiling'])}</span>
            <span class="quick-chip">Target {money(levels['target'])}</span>
            <span class="quick-chip">Exit {money(levels['stop'])}</span>
          ...
          <div class="decision-detail"><strong>Why now:</strong> {reason_text}</div>
          <div class="decision-detail">Wider market: <strong>{mood['label']}</strong></div>
          <div class="decision-detail"><strong>Paper/watch only.</strong> v13 is still in prospective forward testing; this is not an evidence-backed real-money trade signal.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_fast_no(best: dict | None, mood: dict):
    calibration_ready = bool(CALIBRATION.get("ready_for_alerts", False))
    if not calibration_ready:
        status = CALIBRATION.get("status", "missing")
        detail = (
            "Clarity's historical evidence model is not ready for live alerts yet. "
            f"Current calibration status: <strong>{status}</strong>. Run the Evidence Calibration workflow before relying on alerts."
        )
    elif best is None:
        detail = "Clarity could not read enough intraday data right now."
    else:
        evidence = best.get("evidence", {})
        strength = evidence.get("evidence_percentile")
        line = evidence.get("threshold_percentile")
        if strength is not None and line is not None:
            detail = (
                f"Closest right now: <strong>{best['ticker']}</strong> · evidence strength "
                f"<strong>{percentile_text(strength)}</strong> · alert line <strong>{percentile_text(line)}</strong>."
            )
        else:
            detail = f"Closest right now: <strong>{best['ticker']}</strong>, but its historical evidence strength could not be calculated."
    ...
    st.markdown(
        ...
        <div class="decision-card no">
          ...
          <div class="decision-detail">Wider market: <strong>{mood['label']}</strong>. Clarity only interrupts you when the live setup clears a threshold chosen on earlier history and survives a later untouched test.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def stock_scan(tickers: list[str]):
    ...
    for ticker in tickers:
        ...
        intra_df = intra.get(ticker, pd.DataFrame())
        ...
        evidence = calibrated_signal(daily_df, intra_df, spy, CALIBRATION)
        if opp is None or quick is None:
            continue

        probability = evidence.get("probability") if evidence.get("available") else None
        rank_value = float(probability) if probability is not None else (.45 * opp + .55 * quick["score"]) / 100.0
        rows.append(
            {
                "ticker": ticker,
                "opportunity": opp,
                ...
                "watch_ready": bool(
                    ...
                    and evidence.get("directional_ok", False)
                    and probability is not None
                    and float(probability) >= float(evidence.get("threshold", 1.0))
                ),
                "combined": rank_value,
            }
        )
...
def render_trade_card(item: dict):
    ...
    evidence = item.get("evidence", {})
    ready = bool(item.get("ready", evidence.get("ready", False)))

    watch_ready = bool(item.get("watch_ready", False))
    css = "green-card" if ready else "yellow-card"
    if ready:
        label = "EVIDENCE LINE CLEARED"
    elif watch_ready:
        label = "PAPER WATCH — V13 LINE CLEARED"
    else:
        label = "WATCH — NOT READY"
...
with today_tab:
    ...
    if status["open"]:
        if not ranked:
            ...
            best = ranked[0]
            ready = bool(best.get("ready", False))
            watch_ready = bool(best.get("watch_ready", False))

            if ready:
                render_fast_yes(best, mood)
            elif watch_ready:
                render_fast_watch(best, mood)
            else:
...
with check_tab:
    ...
    if ticker:
        with st.spinner(f"Checking {ticker}..."):
            ...
            opp = daily_opportunity_completed(daily.get(ticker, pd.DataFrame()), as_of=intra.get(ticker, pd.DataFrame()).index[-1] if not intra.get(ticker, pd.DataFrame()).empty else None)
        ...
        if opp is None or quick is None:
            ...
            item = {
                ...
                "watch_ready": bool(
                    evidence.get("available")
                    and evidence.get("directional_ok", False)
                    and evidence.get("probability") is not None
                    and float(evidence.get("probability")) >= float(evidence.get("threshold", 1.0))
                ),
                "combined": evidence.get("probability", 0.0) if evidence.get("available") else (.45 * opp + .55 * quick["score"]) / 100.0,
            }
            ...
            if evidence.get("available"):
                test = evidence.get("test_metrics", {})
                st.markdown(
                    f"""
                    <div class="card">
                      <div class="muted">Historical evidence check</div>
                      <div class="big">{percentile_text(evidence.get('evidence_percentile'))}</div>
                      <div class="muted">Current alert line: {percentile_text(evidence.get('threshold_percentile'))} · {evidence['grade']}</div>
                      <div class="muted">Untouched test: {test.get('alerts', 0)} separated alerts · positive after assumed drag {pct_text(test.get('positive_rate'))} · average {test.get('avg_r', 0):+.2f}R · stress {test.get('stress_avg_r', 0):+.2f}R</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            else:
                st.info("Historical calibration has not finished yet, so the Opportunity and Quick Move scores are descriptive only.")
...
with settings_tab:
    ...
    st.write("**Historical evidence engine**")
    cal_status = CALIBRATION.get("status", "missing")
    if CALIBRATION.get("ready_for_alerts", False):
        test = CALIBRATION.get("metrics", {}).get("test", {})
        base = test.get("baseline", {})
        st.success(
            f"Ready · alert line {percentile_text(CALIBRATION.get('threshold_percentile'))} · "
            f"untouched test {test.get('alerts', 0)} separated alerts across {test.get('alert_days', 0)} days · "
            f"positive {pct_text(test.get('positive_rate'))} · average {test.get('avg_r', 0):+.2f}R · "
            f"stress {test.get('stress_avg_r', 0):+.2f}R"
        )
        if test.get("positive_rate_lift") is not None:
            st.caption(
                f"Compared with matched snapshots in the same untouched period: "
                f"{100*test.get('positive_rate_lift', 0):+.1f} percentage-point improvement in positive outcomes."
            )
        st.caption(
            f"Base execution-drag scenario: {CALIBRATION.get('round_trip_cost_bps', '?')} bps round trip · "
            f"stress scenario: {CALIBRATION.get('stress_round_trip_cost_bps', '?')} bps. "
            "These are assumptions for testing, not measured Robinhood costs."
        )
        st.caption(
            f"Last calibrated: {CALIBRATION.get('generated_at', 'unknown')} · "
            f"history window: {CALIBRATION.get('training_window_calendar_days', '?')} calendar days · "
            f"split by complete trading days."
        )
    else:
        st.warning(f"Evidence engine status: {cal_status}. Evidence-backed trade alerts stay off; Clarity Watch paper alerts can still run during the forward test.")
        gates = CALIBRATION.get("readiness_gates", [])
        failed = [g.get("name") for g in gates if not g.get("passed")]
        if failed:
            st.caption("Still needs to pass: " + " · ".join(str(x) for x in failed[:4]))

    st.write("**Live forward test**")
    forward = load_forward_summary()
    if forward:
        settled_n = int(forward.get("settled_events", 0) or 0)
        days_n = int(forward.get("distinct_settled_days", 0) or 0)
        avg_base = forward.get("average_net_r_base")
        avg_stress = forward.get("average_net_r_stress")
        med_base = forward.get("median_net_r_base")
        pos_base = forward.get("positive_rate_base")
        pos_stress = forward.get("positive_rate_stress")
        base_lcb = forward.get("day_lcb80_r")
        stress_lcb = forward.get("stress_day_lcb80_r")
        fstatus = str(forward.get("status", "collecting"))

        if fstatus == "forward_evidence_stronger":
            st.success("Forward status: stronger prospective evidence. Base and stress day-level checks are both positive.")
        elif fstatus == "promising_base_case":
            st.info("Forward status: promising base case. Normal-cost results have passed the first review; stress-cost robustness is not established yet.")
        elif fstatus == "mixed_first_review":
            st.warning("Forward status: mixed. There are enough observations for a first review, but the base-case day-level evidence has not cleared zero.")
        else:
            st.info("Forward status: collecting genuinely new observations.")

        st.markdown(
            f"""
            <div class="card">
              <div class="big">{settled_n} settled watch events · {days_n} trading days</div>
              <div class="muted" style="margin-top:.35rem;">
                Average base result: <strong>{r_text(avg_base)}</strong> ·
                median: <strong>{r_text(med_base)}</strong> ·
                positive: <strong>{pct_text(pos_base)}</strong>
              </div>
              <div class="muted">
                Stress result: <strong>{r_text(avg_stress)}</strong> ·
                positive: <strong>{pct_text(pos_stress)}</strong>
              </div>
              <div class="muted">
                Day-level 80% lower bound: base <strong>{r_text(base_lcb)}</strong> ·
                stress <strong>{r_text(stress_lcb)}</strong>
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        by_horizon = forward.get("by_horizon", {}) or {}
        if by_horizon:
            bits = []
            for h in sorted(by_horizon, key=lambda x: int(x)):
                m = by_horizon[h]
                bits.append(
                    f"{h} min: {m.get('events', 0)} events · "
                    f"{r_text(m.get('average_net_r_base'), 2)} base · "
                    f"{r_text(m.get('average_net_r_stress'), 2)} stress"
                )
            st.caption("Horizon check — " + " | ".join(bits))

        st.caption(
            "Clarity Watch notifications are observational/paper alerts. The forward test remains separate from real-money evidence-backed alerts."
        )
    else:
        st.caption("Forward-test summary is not reachable right now. Shadow logging can still continue in GitHub Actions.")

    st.caption(
        "v13 evaluates separated alert events, chooses a 15-, 30-, or 60-minute horizon using validation history, and adds market-structure context. Opportunity and Quick Move remain explanatory only."
    )
