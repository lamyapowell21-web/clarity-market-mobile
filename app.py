    ...
    st.markdown(
        ...
        <div class="decision-card yes">
          ...
          <div class="quick-line">
            <span class="quick-chip">Historical fit {100*probability:.0f}%</span>
            <span class="quick-chip">Evidence line {100*threshold:.0f}%</span>
          ...
          <div class="decision-detail">Untouched historical test: <strong>{test_metrics.get('alerts', 0)} alerts</strong> · target-first rate <strong>{(100*test_metrics.get('target_hit_rate', 0)):.0f}%</strong> · average <strong>{test_metrics.get('avg_r', 0):+.2f}R</strong></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_fast_no(best: dict | None, mood: dict):
    ...
    if not calibration_ready:
        ...
        threshold = best.get("evidence", {}).get("threshold")
        if p is not None and threshold is not None:
            detail = (
                f"Closest right now: <strong>{best['ticker']}</strong> · historical fit "
                f"<strong>{100*p:.0f}%</strong> · evidence line <strong>{100*threshold:.0f}%</strong>."
            )
        else:
            detail = f"Closest right now: <strong>{best['ticker']}</strong>, but its historical fit could not be calculated."
            ...
            if evidence.get("available"):
                ...
                st.markdown(
                    ...
                    <div class="card">
                      ...
                      <div class="big">{100*evidence['probability']:.0f}% setup fit</div>
                      <div class="muted">Current alert line: {100*evidence['threshold']:.0f}% · {evidence['grade']}</div>
                      <div class="muted">Untouched test: {test.get('alerts', 0)} alerts · target-first {100*test.get('target_hit_rate', 0):.0f}% · average {test.get('avg_r', 0):+.2f}R</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
...
with settings_tab:
    ...
    if CALIBRATION.get("ready_for_alerts", False):
        ...
        st.success(
            f"Ready · evidence line {100*CALIBRATION.get('probability_threshold', 0):.0f}% · "
            f"untouched test {test.get('alerts', 0)} alerts · "
            f"target-first {100*test.get('target_hit_rate', 0):.0f}% · average {test.get('avg_r', 0):+.2f}R"
        )
        st.caption(f"Last calibrated: {CALIBRATION.get('generated_at', 'unknown')} · history window: {CALIBRATION.get('training_window_days', '?')} days")
    else:
        st.warning(f"Evidence engine status: {cal_status}. Live evidence-backed alerts stay off until calibration passes its later untouched test.")

    st.caption(
        "The older Opportunity and Quick Move scores remain on screen to explain what the market is doing, but they no longer decide whether Clarity interrupts you."
    )
