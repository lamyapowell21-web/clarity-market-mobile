...
def session_close_for(ts) -> pd.Timestamp | None:
    try:
        ...
        schedule = nyse.schedule(start_date=stamp.date(), end_date=stamp.date())
        if schedule.empty:
            return None
        return schedule.iloc[0]["market_close"].tz_convert(NY)
    except Exception:
        return None


def full_horizon_fits(signal_ts, horizon_minutes: int) -> bool:
    close = session_close_for(signal_ts)
    if close is None:
        return False
    ...
    if ts.tzinfo is None:
        ...
        ts = ts.tz_convert(NY)
    return ts + pd.Timedelta(minutes=int(horizon_minutes)) <= close


def send_watch_notification(row: dict, config: dict) -> None:
    if not bool(config.get("watch_alerts_enabled", True)):
        return
    topic = os.environ.get("NTFY_TOPIC", "").strip()
    if not topic:
        print("NTFY_TOPIC is not set; shadow event recorded without a phone notification.")
        return

    ticker = str(row["ticker"])
    evidence = _f(row.get("evidence_percentile"), np.nan)
    evidence_text = f"{evidence:.0f}th percentile" if np.isfinite(evidence) else "threshold crossed"
    horizon = int(float(row.get("horizon_minutes", 15)))
    entry = _f(row.get("entry_price"), np.nan)
    entry_text = f"${entry:,.2f}" if np.isfinite(entry) else "current market snapshot"

    payload = {
        "topic": topic,
        "title": f"💎 CLARITY WATCH — {ticker}",
        "message": (
            f"Paper/watch only · Evidence {evidence_text} · "
            f"{horizon}-minute research window · snapshot {entry_text}. "
            "Clarity is still in prospective forward testing."
        ),
        "priority": 3,
    }
    app_url = os.environ.get("CLARITY_APP_URL", "").strip()
    if app_url:
    ...
    try:
        ...
        r.raise_for_status()
        print(f"Clarity Watch notification sent for {ticker}.")
    except Exception as exc:
        # Logging must continue even if ntfy is temporarily unavailable.
        print(f"Watch notification failed for {ticker}: {exc}")
...
def settle_pending(events: pd.DataFrame, intraday: dict[str, pd.DataFrame], config: dict) -> tuple[pd.DataFrame, int]:
    ...
    for i in events.index:
        ...
        if not full_horizon_fits(signal_ts, horizon):
            close = session_close_for(signal_ts)
            events.at[i, "status"] = "incomplete_session"
            events.at[i, "outcome_timestamp_et"] = close.isoformat() if close is not None else ""
            events.at[i, "ambiguous"] = "false"
            changed += 1
            continue
        ...
        events.at[i, "max_downside"] = f"{float(base.get('max_drawdown', np.nan)):.10f}"
...
def _day_stats(settled: pd.DataFrame, column: str) -> tuple[float | None, float | None, float | None]:
    if settled.empty:
        return None, None, None
    tmp = settled.copy()
    tmp[column] = pd.to_numeric(tmp[column], errors="coerce")
    daily = tmp.groupby("session_date")[column].mean().dropna()
    if daily.empty:
        return None, None, None
    ...
    return mean, se, float(mean - 0.84 * se)


def summarize(events: pd.DataFrame, calibration: dict) -> dict:
    status_col = events["status"].astype(str).str.lower() if not events.empty else pd.Series(dtype=str)
    settled = events[status_col == "settled"].copy() if not events.empty else _blank_events()
    ambiguous = int((status_col == "ambiguous").sum()) if not events.empty else 0
    pending = int((status_col == "pending").sum()) if not events.empty else 0
    incomplete = int((status_col == "incomplete_session").sum()) if not events.empty else 0

    summary = {
        "version": 2,
        "method": "v13_live_watch_forward_test",
        "calibration_method": calibration.get("method"),
        "calibration_generated_at": calibration.get("generated_at"),
        ...
        "incomplete_session_events": incomplete,
        "distinct_settled_days": 0,
        "status": "collecting",
        "note": "Clarity Watch is paper/research mode. Forward statistics do not enable real-money trade recommendations.",
    }

    if settled.empty:
        return summary

    for c in ["net_r_base", "net_r_stress", "final_return", "horizon_minutes"]:
        settled[c] = pd.to_numeric(settled[c], errors="coerce")

    valid = settled["net_r_base"].notna()
    s = settled.loc[valid].copy()
    base = s["net_r_base"]
    stress = s["net_r_stress"]
    final_ret = s["final_return"]

    day_avg, day_se, day_lcb = _day_stats(s, "net_r_base")
    stress_day_avg, stress_day_se, stress_day_lcb = _day_stats(s, "net_r_stress")
    ...
    summary.update({
        ...
        "positive_rate_stress": float((stress > 0).mean()) if len(stress) else None,
        "average_net_r_base": float(base.mean()) if len(base) else None,
        "median_net_r_base": float(base.median()) if len(base) else None,
        "total_net_r_base": float(base.sum()) if len(base) else None,
        "average_net_r_stress": float(stress.mean()) if len(stress) else None,
        "median_net_r_stress": float(stress.median()) if len(stress) else None,
        "total_net_r_stress": float(stress.sum()) if len(stress) else None,
        "positive_final_return_rate": float((final_ret > 0).mean()) if final_ret.notna().any() else None,
        "day_average_net_r_base": day_avg,
        "day_se_r": day_se,
        "day_lcb80_r": day_lcb,
        "stress_day_average_net_r": stress_day_avg,
        "stress_day_se_r": stress_day_se,
        "stress_day_lcb80_r": stress_day_lcb,
    })

    by_horizon = {}
    for horizon, g in s.groupby("horizon_minutes"):
        if pd.isna(horizon):
            continue
        by_horizon[str(int(horizon))] = {
            "events": int(len(g)),
            "positive_rate_base": float((g["net_r_base"] > 0).mean()),
            "positive_rate_stress": float((g["net_r_stress"] > 0).mean()),
            "average_net_r_base": float(g["net_r_base"].mean()),
            "average_net_r_stress": float(g["net_r_stress"].mean()),
        }
    summary["by_horizon"] = by_horizon

    min_events = int(load_config(BASE / "cloud_config.json").get("forward_review_min_events", 20))
    min_days = int(load_config(BASE / "cloud_config.json").get("forward_review_min_days", 5))
    stronger_events = int(load_config(BASE / "cloud_config.json").get("forward_stronger_review_min_events", 40))
    stronger_days = int(load_config(BASE / "cloud_config.json").get("forward_stronger_review_min_days", 10))

    if len(s) >= min_events and s["session_date"].nunique() >= min_days:
        if (
            summary["average_net_r_base"] is not None
            and summary["average_net_r_base"] > 0
            and summary["day_lcb80_r"] is not None
            and summary["day_lcb80_r"] > 0
        ):
            summary["status"] = "promising_base_case"
        else:
            summary["status"] = "mixed_first_review"

    if len(s) >= stronger_events and s["session_date"].nunique() >= stronger_days:
        if (
            summary["average_net_r_base"] > 0
            and summary["average_net_r_stress"] > 0
            and summary["day_lcb80_r"] is not None
            and summary["day_lcb80_r"] > 0
            and summary["stress_day_lcb80_r"] is not None
            and summary["stress_day_lcb80_r"] > 0
        ):
            summary["status"] = "forward_evidence_stronger"
...
def main() -> None:
    ...
    if market.get("open"):
        ...
        for ticker in watchlist:
            ...
            if not sig or not sig.get("available") or sig.get("probability") is None:
                continue
            signal_ts = pd.Timestamp(sig["features"]["timestamp"])
            horizon = int(calibration.get("horizon_minutes", 15))
            if not full_horizon_fits(signal_ts, horizon):
                continue
        ...
        if candidates:
            ...
            if row["event_id"] not in existing:
                ...
                changed = True
                send_watch_notification(row, config)
                print(
    ...
    new_summary_text = json.dumps(summarize(events, calibration), indent=2)
    if new_summary_text != previous_summary:
