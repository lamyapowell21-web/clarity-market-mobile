...
def intraday_features(df: pd.DataFrame, spy_df: pd.DataFrame | None = None) -> dict | None:
    """Session-aware intraday features.

    v12 avoids treating the overnight gap as a 15/60-minute move and normalizes
    volume against roughly the same time of day on prior sessions when possible.
    That matters because U.S. intraday volume has a strong time-of-day pattern.
    """
    if df is None or df.empty:
        return None
    ...
    session = d[d.index.date == current_date].copy()
    if len(session) < 4:
        return None

    close = session["Close"]
    volume = session["Volume"].fillna(0) if "Volume" in session else pd.Series(0, index=session.index)
    price = float(close.iloc[-1])

    # 15-minute momentum never crosses the overnight boundary.
    ret15 = float(price / close.iloc[-4] - 1)

    # Before a full hour has elapsed, use the current-session move so the feature
    # remains session-local instead of reaching into yesterday's close.
    if len(close) >= 13:
        ret60 = float(price / close.iloc[-13] - 1)
    else:
        ret60 = float(price / close.iloc[0] - 1)
    ...
    # Compare recent volume with the same clock-time slots on prior sessions.
    # This reduces false 'volume surge' signals caused by the normal opening/closing U-shape.
    prior = d[d.index.date < current_date]
    recent = session.tail(3)
    baselines = []
    if "Volume" in prior and not prior.empty:
        prior_vol = pd.to_numeric(prior["Volume"], errors="coerce")
        prior_slots = np.asarray([_minutes_from_open(x) for x in prior.index], dtype=float)
        for rts in recent.index:
            slot = _minutes_from_open(rts)
            mask = np.abs(prior_slots - slot) <= 2.5
            same_slot = prior_vol.iloc[np.where(mask)[0]].dropna().tail(10)
            if len(same_slot) >= 3 and float(same_slot.mean()) > 0:
                baselines.append(float(same_slot.mean()))

    recent_vol = float(pd.to_numeric(recent.get("Volume", pd.Series(0, index=recent.index)), errors="coerce").fillna(0).mean())
    if baselines:
        normal_vol = float(np.mean(baselines))
    elif len(session) >= 7:
        normal_vol = float(volume.iloc[:-3].tail(24).mean())
    elif "Volume" in prior and not prior.empty:
        normal_vol = float(pd.to_numeric(prior["Volume"], errors="coerce").dropna().tail(24).mean())
    else:
        normal_vol = 0.0
    vr = recent_vol / normal_vol if normal_vol > 0 else 1.0

    # Use today's own price action for the short-term ceiling whenever enough bars exist.
    if len(session) >= 6:
        prior_high = float(session["High"].iloc[-21:-1].max())
    else:
        prior_high = float(d["High"].iloc[-21:-1].max())

    mid = float(close.tail(20).mean())
    bar_range = float((session["High"] - session["Low"]).tail(12).mean())
    ...
    if spy_df is not None and not spy_df.empty and "Close" in spy_df:
        sp = _numeric_frame(spy_df).dropna(subset=["Close"])
        sp_idx = pd.to_datetime(sp.index)
        if sp_idx.tz is None:
            sp_idx = sp_idx.tz_localize(NY)
        else:
            sp_idx = sp_idx.tz_convert(NY)
        sp.index = sp_idx
        sp_session = sp[sp.index.date == current_date]
        sp_close = pd.to_numeric(sp_session["Close"], errors="coerce").dropna()
        if len(sp_close) >= 4:
            market_ret15 = float(sp_close.iloc[-1] / sp_close.iloc[-4] - 1)
    ...
    return {
        ...
        "minutes_from_open": _minutes_from_open(current_ts),
...
def calibrated_signal(daily_df: pd.DataFrame, intraday_df: pd.DataFrame, spy_df: pd.DataFrame, calibration: dict) -> dict:
    ...
    if int(calibration.get("version", 0) or 0) < 2:
        return {
            "available": False,
            "ready": False,
            "reason": "Clarity v12 needs a fresh evidence calibration.",
            "features": row,
        }
